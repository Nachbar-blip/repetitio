...
with sync_playwright() as p:
    ...
    for name, w in [("desktop", 1200), ("phone", 400)]:
        ...
        for deck, cat in CASES:
            ...
            ea = pg.evaluate("window.REPETITIO.filteredCards.findIndex(c => c.niveau === 'ea')")
            pg.evaluate(f"window.REPETITIO.showCard({ea})"); pg.wait_for_timeout(300)
            pg.screenshot(path=OUT / f"{deck}-front-{name}.png", full_page=True)
            ...
            pg.screenshot(path=OUT / f"{deck}-back-{name}.png", full_page=True)
